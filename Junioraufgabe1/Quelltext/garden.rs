...
impl Garden {
    ...
    fn calc_square_size(&self, vertical_lines: i64) -> f64 {
        let new_horizontal_lines = self.interested / 2 - vertical_lines;

        print!("({} / {}) / ({} / {}) = ", self.width, new_horizontal_lines, self.height, vertical_lines);

        if new_horizontal_lines == 0 {
            println!("{}", (self.width) as f64 / (self.height / vertical_lines) as f64);
            return (self.width) as f64 / (self.height / vertical_lines) as f64;
        }

        println!("{}", (self.width / new_horizontal_lines) as f64 / (self.height / vertical_lines) as f64);

        ((self.width / new_horizontal_lines) as f64 / (self.height / vertical_lines) as f64)
    }
    ...
    fn calc_most_square_layout(&self, last_vertical_lines: i64, last_square_diff: f64) -> (i64, i64) {
        ...
        let new_square_diff = self.calc_square_size(new_vertical_lines);

        println!("{}", new_square_diff);

        println!("{}", is_more_squared(new_square_diff, last_square_diff));
        ...
        if is_more_squared(new_square_diff, last_square_diff) {
            ...
            (
                last_vertical_lines,
                self.interested / 2 - last_vertical_lines
            )
        }
    }
}
